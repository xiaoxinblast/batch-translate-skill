...
> ⚠️ **强制步骤**：语境分析报告若含「疑似术语库未覆盖的专名」清单（内容非"无"），必须在开译前处理。否则这些专名会被翻译 Agent 就近臆测（如特定角色名被 Agent 凭上下文臆测出一个看似合理但错误的译名），且校对 Agent 只对"是否符合术语库"负责、术语库缺失时无法纠正。
...
> 1. **定义句后缺主语**：定义句之后的分句，是否补出了主语？若直接以动词/介词开头则为错误
