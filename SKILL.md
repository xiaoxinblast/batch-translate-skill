...
向用户展示扫描结果，然后用 `AskUserQuestion`（multiSelect）依次询问：
...
#### 编译要求
- 多来源时去重、整理为统一结构
- **古风/武侠小说语感必须在风格指南中占据显著位置**，附正反对比示例
- **必须包含以下硬性格式规则**：
...
**选项 B（从源文件导入）**：
> ⚠️ 仅导入**日语源文与中文译文匹配的条目**。如发现译文可能过时（如源文含 flavor text 修订标记），跳过该条目并在报告中标注。
...
>   ① 定义句之后的分句，补出主语（"它/其/该××"），禁止直接以动词或介词开头
...
> 
> **说明文体裁专项检查**（图鉴、道具描述、角色介绍等"定义句+动作罗列"结构必须执行）：
> 1. **定义句后缺主语**：定义句之后的分句，是否补出了主语？若直接以动词/介词开头则为错误。例如"向周围喷洒熔岩"→ 应为"**它会**向周围喷洒熔岩"
> 2. **连动缺连接词**：连续动作之间是否有"并/且/还"？例如"锁定猎物，发射光线"→ 应为"锁定猎物，**并**发射光线"
> 3. **条件分句缺所指**：以"当/在/处于……时"开头的分句，是否补出了指代对象？例如"处于鳞粉缠绕状态时"→ 应为"**在其**处于鳞粉缠绕状态时"
