...
①主语缺了要补 ②汉字词别照搬 ③语序改 SVO ④被动转主动 ⑤长句拆短 ⑥长定语后置 ⑦暧昧适度去 ⑧指示词具体化 ⑨敬语靠措辞弥合
...
1. 定义句之后的分句，禁止直接以动词或介词开头。**先通过 note、context、项目参考文件、联网搜索等方式查证描述对象的身份，再选择恰当的主语补出。** 未明确时禁用"他/她/它"，改用中性表达
...
- `tm_fragments`：片段匹配。`fragment_target` 为提取到的对应中文片段，按 `fragment_target_confidence` 分级使用：
  - **high**：`fragment_target` 为精对齐结果，可直接复用
  - **medium**：`fragment_target` 为近似区间，参考时以 `match_target` 完整语义为准
  - **full_sentence**：片段即整句，直接参考 `match_target`
